class Crafting {
    ...
    craftItem(recipeName) {
      ...
      if (recipe) {
        if (this.playerSkills.craftingLevel >= recipe.skillLevel) {
          const materials = recipe.materials;
          for (const material of materials) {
            const materialInInventory = this.materials.find((materialInInventory) => materialInInventory.name === material.name);
            if (materialInInventory) {
              if (materialInInventory.quantity >= material.quantity) {
                materialInInventory.quantity -= material.quantity;
              } else {
                console.log(`Not enough ${material.name} to craft ${recipeName}`);
                return;
              }
            } else {
              console.log(`Unknown material: ${material.name}`);
              return;
            }
          }
          ...
          console.log(`You don't have the necessary skill level to craft ${recipeName}`);
        }
      } else {
    ...
    upgradeCraftingSpeed() {
      this.playerSkills.craftingSpeed += 0.1;
    }

    upgradeCraftingLevel() {
        this.playerSkills.craftingLevel += 1;
      }
    
      getCraftingTime(recipeName) {
        const recipe = this.recipes.find((recipe) => recipe.name === recipeName);
        if (recipe) {
          return recipe.craftingTime / this.playerSkills.craftingSpeed;
        } else {
          console.log(`Unknown recipe: ${recipeName}`);
        }
      }
      ...
      getCraftingXP() {
        return this.playerSkills.craftingXP;
      }
    
      getCraftingLevel() {
        return this.playerSkills.craftingLevel;
      }
    
      getCraftingSpeed() {
        return this.playerSkills.craftingSpeed;
      }
      ...
      addItem(itemName, quantity) {
        ...
        if (item) {
          ...
          this.inventory.push({ name: itemName, quantity: quantity });
        }
      }
      ...
      removeItem(itemName, quantity) {
        ...
        if (item) {
          if (item.quantity >= quantity) {
            ...
            console.log(`You don't have enough ${itemName} to remove`);
          }
        } else {
          console.log(`Unknown item: ${itemName}`);
        }
      ...
      craftAll(recipeName) {
        ...
        if (recipe) {
          ...
          for (const material of materials) {
            ...
            if (materialInInventory) {
              if (materialInInventory.quantity >= material.quantity) {
                ...
                console.log(`Not enough ${material.name} to craft all ${recipeName}`);
                return;
              }
            } else {
              console.log(`Unknown material: ${material.name}`);
          ...
          console.log(`Crafted all ${recipe.result}`);
          this.playerSkills.craftingLevel += 0.1;
          this.playerSkills.craftingXP += 10;
        } else {
          console.log(`Unknown recipe: ${recipeName}`);
      ...
      addCraftingQueue(recipeName) {
        this.craftingQueue.push(recipeName);
      }
    
      removeCraftingQueue(recipeName) {
        const index = this.craftingQueue.indexOf(recipeName);
        if (index > -1) {
